package org.meristem.oneapp.notificationservice.exceptionHandler;

import org.meristem.oneapp.notificationservice.exceptionHandler.exceptions.BadRequestException;
import org.meristem.oneapp.notificationservice.exceptionHandler.exceptions.ResourceNotFoundException;
...
public class GlobalControllerAdvice implements MessageSourceAware {
    ...
    protected ResponseEntity<ErrorDetails> handleUserNotFoundException(ResourceNotFoundException ex, WebRequest request) {
        return handleExceptionInternal(ex, HttpStatus.NOT_FOUND, request, List.of(ex.getName() + " '" + ex.getResource() + "' not found"));
    }
