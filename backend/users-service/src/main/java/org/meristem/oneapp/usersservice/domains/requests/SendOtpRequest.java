...
@Builder
public record SendOtpRequest(@Schema(anyOf = {MessageSubject.class}, example = "1", allowableValues = {"1", "2", "3", "4", "5", "6", "7"}, description = "use 1 for otp send before registration and 2 for otp sent for password reset, 3 for registration flow") @NotNull(message = "otp type must be passed") @Max(value = 7, message = "Cannot be higher than 7") @Min(value = 1, message = "Cannot be less than 1") Integer otpType,
