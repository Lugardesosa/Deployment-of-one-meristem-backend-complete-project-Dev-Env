...
if command -v pack &> /dev/null; then
  echo "pack CLI already installed — skipping."
else
...
# ----------------------------------------
# STEP 2: Detect newly added backend folders (only report, do NOT add to build)
# ----------------------------------------
for svc in $(ls ${BASE_PATH}); do
  if [ -d "${BASE_PATH}/${svc}" ]; then
    if [[ ! " ${SVC_NAMES[@]} " =~ " ${svc} " ]]; then
      echo "New microservice folder detected: ${svc} — NOT added to build list."
    fi
  fi
done
